...
The script prompts for sudo access and confirmation (y).
...
When prompted with a menu for which method to use when running NuSMV, each method should be run once. Approximate runtimes for each method on this example, along with their corresponding tables in the paper are:
	1. **ENTER RUNTIME HERE** - Table 5
	2. **ENTER RUNTIME HERE** - Table 6
	3. **ENTER RUNTIME HERE** - Table 7
...
Inputs
=============
The `Inputs` directory is used for containing the input files for use with the SSP and ExCov problems. Inputs for the minimal running examples are preloaded in this folder.
...
SSP networks include the error type in their file name:
	B_errortype_...

*ExCov networks need a clear naming format.*

These were used to show correctness of the network descriptions by comparing verification results with those of the valid networks generated by the scripts. This comparision is included in Tables 9 and 11.